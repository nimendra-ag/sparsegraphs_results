"""Monte Carlo CV benchmark for gSpan-CORK + FDDL (GPU).

This is the headline number for the gSpan-CORK arm: mean +/- std over
...
and implements/wl_edge_fddl_gpu_mccv.py also run on — so the gSpan-CORK, node-WL
and edge-WL numbers are produced by identical machinery and only the encoder
differs.
...
Usage
-----
    python implements/gspan_cork_fddl_gpu_mccv.py                 # full run
    python implements/gspan_cork_fddl_gpu_mccv.py --seed 7 --out-dir results/<run>
    python implements/gspan_cork_fddl_gpu_mccv.py --aggregate --out-dir results/<run>
"""
