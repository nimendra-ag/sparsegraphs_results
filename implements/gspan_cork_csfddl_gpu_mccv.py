"""Monte Carlo CV benchmark for gSpan-CORK + CS-FDDL (GPU).

This is the headline number for the gSpan-CORK cost-sensitive arm: mean +/- std
...
implements/gspan_cork_fddl_gpu_mccv.py also runs on — so this and the vanilla
FDDL baseline on the same encoder are produced by identical machinery and only
the dict learner differs.
...
covers gSpan mining plus CORK selection; the `sparse_codes_*` phases cover the
VF2 encoding. CORK runs batched on the GPU and the encoding is parallelised
across cores, but gSpan mining is recursive pure Python and is not parallelised
— if `fit_encoder_dict` dominates, the only levers are `min_support_ratio`,
`max_num_vertices` and `cork_max_features`, all set on the encoder below.

Usage
-----
    python implements/gspan_cork_csfddl_gpu_mccv.py                 # full run
    python implements/gspan_cork_csfddl_gpu_mccv.py --seed 7 --out-dir results/<run>
    python implements/gspan_cork_csfddl_gpu_mccv.py --aggregate --out-dir results/<run>
"""
