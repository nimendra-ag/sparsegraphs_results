"""Monte Carlo CV benchmark for WL + FDDL (GPU).
...
script used to carry inline. It is shared with implements/wl_edge_fddl_gpu_mccv.py
so every arm is produced by identical machinery and only the encoder differs.

Usage
-----
    python implements/wl_fddl_gpu_mccv.py                 # full run
    python implements/wl_fddl_gpu_mccv.py --seed 7 --out-dir results/<run>
    python implements/wl_fddl_gpu_mccv.py --aggregate --out-dir results/<run>
"""
