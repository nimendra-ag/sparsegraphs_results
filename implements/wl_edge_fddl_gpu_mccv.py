"""Monte Carlo CV benchmark for EdgeWL + FDDL (GPU).

This is the headline number for the edge-WL arm: mean +/- std over
...
set, the CSV layout, the resumable per-seed appends) lives in utils/mccv.py,
which implements/wl_fddl_gpu_mccv.py also runs on — so the edge-WL and node-WL
numbers are produced by identical machinery and only the encoder differs.

Usage
-----
    python implements/wl_edge_fddl_gpu_mccv.py            # full run
    python implements/wl_edge_fddl_gpu_mccv.py --seed 7 --out-dir results/<run>
    python implements/wl_edge_fddl_gpu_mccv.py --aggregate --out-dir results/<run>
...
bundles (implements/wl_edge_fddl_gpu.py) care about the salt.
"""
