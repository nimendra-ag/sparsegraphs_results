"""Export the deployable EdgeWL + Bayesian (BPFA, GPU) model bundle.
...
The headline performance for the paper comes from wl_edge_bayesian_gpu_mccv.py
(Monte Carlo CV, mean +/- std over 5 seeds), NOT from this script's single-split
test metrics.

Identical to implements/wl_bayesian_gpu.py except for the encoder: EdgeWL
refines edge labels instead of node subtrees. Everything downstream (selection,
scaling, BPFA, evaluation, bundle layout) is shared code, so the two runs are
directly comparable.
...
`PYTHONHASHSEED=0 python implements/wl_edge_bayesian_gpu.py` if the bundle is
