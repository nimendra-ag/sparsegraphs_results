"""Monte Carlo CV benchmark for EdgeWL + Online DL (scikit-learn).

This is the headline number for the edge-WL + online-DL arm: mean +/- std over
...
breakdown) lives in utils/mccv.py, which implements/wl_online_dl_mccv.py also
runs on — so the edge-WL and node-WL numbers are produced by identical
machinery and only the encoder differs.

Usage
-----
    python implements/wl_edge_online_dl_mccv.py            # full run
    python implements/wl_edge_online_dl_mccv.py --seed 7 --out-dir results/<run>
    python implements/wl_edge_online_dl_mccv.py --aggregate --out-dir results/<run>
...
bundles (implements/wl_edge_online_dl.py) care about the salt.
"""
