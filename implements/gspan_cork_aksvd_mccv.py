"""Monte Carlo CV benchmark for gSpan-CORK + AKSVD.

This is the headline number for the gSpan-CORK + AKSVD arm: mean +/- std over
...
AKSVD's atoms carry no class identity (it is unsupervised), so — exactly as in
implements/wl_aksvd_mccv.py — the SRC arms in the results CSV stay NaN.
...
covers gSpan mining plus CORK selection plus the AKSVD fit; the `sparse_codes_*`
phases cover the VF2 encoding and OMP coding. CORK runs batched on the GPU and
the encoding is parallelised across cores, but gSpan mining is recursive pure
Python and is not parallelised — if `fit_encoder_dict` dominates, the levers are
`min_support_ratio`, `max_num_vertices` and `cork_max_features`, all set on the
encoder below.

Usage
-----
    python implements/gspan_cork_aksvd_mccv.py                 # full run
    python implements/gspan_cork_aksvd_mccv.py --seed 7 --out-dir results/<run>
    python implements/gspan_cork_aksvd_mccv.py --aggregate --out-dir results/<run>
"""
