"""Export the deployable FSM + Online DL (scikit-learn) model bundle.
...
The headline performance for the paper comes from fsm_online_dl_mccv.py (Monte
Carlo CV, mean +/- std over 5 seeds), NOT from this script's single-split test
metrics.

Identical to implements/wl_online_dl.py except for the encoder: FSM vectorises
...
degree sequence) instead of WL subtree hashes. Same sklearn DictionaryLearning
...
CAVEATS
  ...
  * The embedding is a RAW COUNT matrix, not WL's L2-normalised counts, so row
    magnitude scales with molecule size. sklearn's solver minimises squared
    reconstruction error, so large molecules carry proportionally more weight in
    the fit than they do in the WL arms; the MaxAbsScaler in the export pipeline
    acts on the sparse codes, downstream of the dictionary.
  * Online dictionary learning is unsupervised and FSM's trim is unsupervised
    too, so the vocab_train labels that utils/pipeline.fit_encoder_and_dictionary
    forwards are unused by both stages of this arm.
  * COST — `OnlineDL` defaults to the notebook's 8000 atoms, which is a
    multi-hour (potentially multi-day) fit on NCI-sized data. Pass a smaller
    `dimensions` for a quick run; nothing else changes.
