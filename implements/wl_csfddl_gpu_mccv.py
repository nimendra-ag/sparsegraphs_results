"""Monte Carlo CV benchmark for WL + CS-FDDL (GPU).
...
script used to carry inline. It is shared with implements/wl_fddl_gpu_mccv.py
so every arm is produced by identical machinery and only the dict learner
differs — making this run directly comparable to the vanilla FDDL baseline.

Usage
-----
    python implements/wl_csfddl_gpu_mccv.py                 # full run
    python implements/wl_csfddl_gpu_mccv.py --seed 7 --out-dir results/<run>
    python implements/wl_csfddl_gpu_mccv.py --aggregate --out-dir results/<run>
"""
