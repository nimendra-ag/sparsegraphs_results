"""Monte Carlo CV benchmark for FSM + Bayesian dictionary learning (BPFA, GPU).

This is the headline number for the FSM + BPFA arm: mean +/- std over
...
implements/wl_bayesian_gpu_mccv.py and implements/fsm_fddl_gpu_mccv.py also run
on — so this arm is produced by identical machinery and differs from
wl_bayesian_gpu only in the encoder, and from fsm_fddl_gpu only in the
dictionary learner.
...
carry no class identity and the SRC arms are recorded as NaN. FSM's trim is
unsupervised too, so this arm never reads the vocab_train labels before the
classifier stage.
...
covers the ego-signature extraction plus the Gibbs sweeps, and the
`sparse_codes_*` phases cover the inference sweeps (`n_infer_iter`). The encoder
is cheap here (one ego graph per node, no isomorphism testing), so if either
phase dominates the levers are on the learner: `dimensions`, `n_iter`,
`n_infer_iter`, and `infer_batch_size` if VRAM is the binding constraint.

Usage
-----
    python implements/fsm_bayesian_gpu_mccv.py                 # full run
    python implements/fsm_bayesian_gpu_mccv.py --seed 7 --out-dir results/<run>
    python implements/fsm_bayesian_gpu_mccv.py --aggregate --out-dir results/<run>
"""
