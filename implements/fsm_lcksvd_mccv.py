"""Monte Carlo CV benchmark for FSM + LC-KSVD.

This is the headline number for the FSM + LC-KSVD arm: mean +/- std over
...
LC-KSVD is the only supervised stage: FSM's trim never reads the labels (unlike
gspan_cork_lcksvd, where CORK uses them too). As in
implements/wl_lcksvd_mccv.py, LC-KSVD's atoms carry class labels but not
necessarily equal per-class block sizes, so utils/mccv.py's SRC arms (which
assume fixed-stride class blocks) correctly stay NaN.
...
Usage
-----
    python implements/fsm_lcksvd_mccv.py                 # full run
    python implements/fsm_lcksvd_mccv.py --seed 7 --out-dir results/<run>
    python implements/fsm_lcksvd_mccv.py --aggregate --out-dir results/<run>
"""
