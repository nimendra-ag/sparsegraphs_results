"""Export the deployable EdgeWL + AKSVD model bundle.
...
The headline performance for the paper comes from wl_edge_aksvd_mccv.py
(Monte Carlo CV, mean +/- std over 5 seeds), NOT from this script's single-split
test metrics.

Identical to implements/wl_aksvd.py except for the encoder: EdgeWL refines
edge labels instead of node subtrees. Everything downstream (selection, scaling,
AKSVD, evaluation, bundle layout) is shared code, so the two runs are directly
comparable.
...
serving process as e.g. `PYTHONHASHSEED=0 python implements/wl_edge_aksvd.py`
