"""Monte Carlo CV benchmark for WL + Frozen K-SVD (GPU).
...
implements/wl_bayesian_gpu_mccv.py so every arm is produced by identical
machinery and only the dictionary learner differs.
...
96 atoms vs 32 residual atoms per added class), so — as in the CPU arm — the
SRC arms, which slice a dictionary at a fixed stride, are recorded as NaN.

Usage
-----
    python implements/wl_frozen_ksvd_gpu_mccv.py                 # full run
    python implements/wl_frozen_ksvd_gpu_mccv.py --seed 7 --out-dir results/<run>
    python implements/wl_frozen_ksvd_gpu_mccv.py --aggregate --out-dir results/<run>
"""
