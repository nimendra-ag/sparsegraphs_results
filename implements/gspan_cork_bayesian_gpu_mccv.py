"""Monte Carlo CV benchmark for gSpan-CORK + Bayesian dictionary learning (BPFA, GPU).

This is the headline number for the gSpan-CORK + BPFA arm: mean +/- std over
...
implements/wl_bayesian_gpu_mccv.py and implements/gspan_cork_fddl_gpu_mccv.py
also run on — so this arm is produced by identical machinery and differs from
wl_bayesian_gpu only in the encoder, and from gspan_cork_fddl_gpu only in the
dictionary learner.
...
BPFA is unsupervised, so — as in implements/wl_bayesian_gpu_mccv.py — its atoms
carry no class identity and the SRC arms are recorded as NaN. The labels are
used only by CORK's selection.
...
covers gSpan mining plus CORK selection plus the Gibbs sweeps; the
`sparse_codes_*` phases cover the VF2 encoding and the inference sweeps
(`n_infer_iter`). CORK runs batched on the GPU and the encoding is parallelised
across cores, but gSpan mining is recursive pure Python and is not parallelised
— if `fit_encoder_dict` dominates, the levers are `min_support_ratio`,
`max_num_vertices` and `cork_max_features`, all set on the encoder below.

Usage
-----
    python implements/gspan_cork_bayesian_gpu_mccv.py                 # full run
    python implements/gspan_cork_bayesian_gpu_mccv.py --seed 7 --out-dir results/<run>
    python implements/gspan_cork_bayesian_gpu_mccv.py --aggregate --out-dir results/<run>
"""
