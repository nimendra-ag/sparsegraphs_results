"""Monte Carlo CV benchmark for EdgeWL + CS-FDDL (GPU).

This is the headline number for the edge-WL cost-sensitive arm: mean +/- std
...
timing breakdown — lives in utils/mccv.py, shared with
implements/wl_edge_fddl_gpu_mccv.py so every arm is produced by identical
machinery and only the dict learner differs — making this run directly
comparable to the vanilla FDDL baseline on the same encoder.
...
bundles (implements/wl_edge_csfddl_gpu.py) care about the salt.

Usage
-----
    python implements/wl_edge_csfddl_gpu_mccv.py                 # full run
    python implements/wl_edge_csfddl_gpu_mccv.py --seed 7 --out-dir results/<run>
    python implements/wl_edge_csfddl_gpu_mccv.py --aggregate --out-dir results/<run>
"""
