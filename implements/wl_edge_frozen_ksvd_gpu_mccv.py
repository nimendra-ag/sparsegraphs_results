"""Monte Carlo CV benchmark for EdgeWL + Frozen K-SVD (GPU).

This is the headline number for the edge-WL + frozen-K-SVD arm: mean +/- std
...
breakdown) lives in utils/mccv.py, which
implements/wl_frozen_ksvd_gpu_mccv.py also runs on — so the edge-WL and node-WL
numbers are produced by identical machinery and only the encoder differs.
...
Frozen K-SVD is supervised, but its dictionary blocks are not equal-width (base
96 atoms vs 32 residual atoms per added class), so — as in the node-WL arm — the
SRC arms, which slice a dictionary at a fixed stride, are recorded as NaN.

Usage
-----
    python implements/wl_edge_frozen_ksvd_gpu_mccv.py            # full run
    python implements/wl_edge_frozen_ksvd_gpu_mccv.py --seed 7 --out-dir results/<run>
    python implements/wl_edge_frozen_ksvd_gpu_mccv.py --aggregate --out-dir results/<run>
...
bundles (implements/wl_edge_frozen_ksvd_gpu.py) care about the salt.
"""
